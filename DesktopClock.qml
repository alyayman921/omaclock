...
// but above the wallpaper. All of its settings live in config.json, which the
...
Item {
    ...
    // --- control API (used by the bar widget) ----------------------------------
    ...
    // ---- persistence ----------------------------------------------------------
    // ---- clock windows (bottom layer, one per screen) -------------------------
    ...
    readonly property string activeFont: root.settings.fontFamily === "system" ? "" : (root.settings.fontFamily ? root.settings.fontFamily : "Inter Variable")
    ...
    readonly property color activeColor: {
        ...
        if (mode === "custom" && explicit)
            return Qt.color(explicit);

        if (mode === "theme")
            return root.roleColor(root.colorRole);
    ...
    readonly property real xRatio: (typeof root.settings.xRatio === "number") ? root.settings.xRatio : 0.5
    readonly property real yRatio: (typeof root.settings.yRatio === "number") ? root.settings.yRatio : (root.settings.position === "top" ? 0.14 : root.settings.position === "bottom" ? 0.86 : 0.5)
    readonly property real fontScale: (typeof root.settings.fontScale === "number") ? root.settings.fontScale : 0.2
    readonly property real clockOpacity: (typeof root.settings.opacity === "number") ? root.settings.opacity : 0.92

    function applyRaw(raw) {
        if (!raw || String(raw).trim().length === 0) {
            console.warn("omaclock applyRaw EMPTY t=", Date.now() % 100000);
            root._configApplied = true;
            return ;
        }
        if (raw === root._lastRaw || raw === root._lastWritten) {
            console.warn("omaclock applyRaw SKIP t=", Date.now() % 100000, raw.length);
            root._configApplied = true;
            return ;
        }
        root._lastRaw = raw;
        try {
            ...
            for (var p in parsed) if (parsed.hasOwnProperty(p)) {
                merged[p] = parsed[p];
            }
            root.settings = merged;
            console.warn("omaclock applied t=", Date.now() % 100000, "x", merged.xRatio, "y", merged.yRatio);
            root._configApplied = true;
        } catch (e) {
            root._configApplied = true;
    ...
    function resetLayout() {
        root.setField("xRatio", 0.5);
        root.setField("yRatio", 0.5);
        root.setField("fontScale", 0.2);
        root.setField("opacity", 0.92);
        root.saveConfig();
    }
    ...
    function saveConfig() {
        ...
        var json = JSON.stringify({
            ...
            "fontScale": Number(num(s.fontScale, 0.2).toFixed(4)),
            "letterSpacing": s.letterSpacing,
            "colorMode": String(s.colorMode || "auto"),
            "colorRole": String(s.colorRole || "bar.text"),
            "color": String(s.color || ""),
            "opacity": Number(num(s.opacity, 0.92).toFixed(2)),
            "position": s.position,
    ...
    Component.onCompleted: console.warn("omaclock ROOT onCompleted t=", Date.now() % 100000)
    ...
    Process {
        ...
        Component.onCompleted: {
            console.warn("omaclock svc onCompleted t=", Date.now() % 100000);
            running = true;
        }
        onExited: root._configApplied = true

        stdout: StdioCollector {
            onStreamFinished: root.applyRaw(text)
        }

    }

    // Fast startup retry so the clock's position is known within ~150ms even if
    // the initial read races shell boot; the 2s poll below keeps external edits
    ...
    Timer {
        ...
        onTriggered: {
            if (!cfgProc.running && !saveProc.running)
        ...
        PanelWindow {
            ...
            Item {
                ...
                Text {
                    ...
                    font.pixelSize: Math.max(8, Math.round(surface.height * root.fontScale))
